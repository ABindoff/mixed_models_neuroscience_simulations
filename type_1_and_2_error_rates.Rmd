...
# method for extracting a legend to use in grid.arrange
...
tests <- function(x, nested = FALSE){
  m1 <- anova(k <- lm(y ~ condition, data = x))
  m1$effect_size <- diff(coef(k))/sd(resid(k))
  m2 <- anova(k <- lmer(y ~ condition + (1|animal.id), data = x))
  m2$effect_size <- diff(fixef(k))/sd(resid(k))
  m3 <- m2
  if(nested){
  ...
  return(cbind(m1$`Pr(>F)`[1], m2$`Pr(>F)`, m3$`Pr(>F)`, m1$effect_size[1], m2$effect_size))
}
```
...
sims.meta$lm <- x[,1]
sims.meta$lmer <- x[,2]
sims.meta$lm.d <- x[,4] - sims.meta$fixed.effects
sims.meta$lmer.d <- x[,5] - sims.meta$fixed.effects
...
Pilot studies are often used to estimate effect sizes. Knowing something about the variance of the random effects and the residual errors can tell us something about how reliable our estimated effect sizes might be.  


```{r}
p1 <- ggplot(sims.meta, aes(x = factor(fixed.effects), y = lm.d, colour = factor(sd.random))) +
  geom_quasirandom(dodge.width = 1/2, alpha = 1/5, size = 2) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  theme(legend.position = "none") +
  ylab("lm.d - true effect")
p2 <- ggplot(sims.meta, aes(x = factor(fixed.effects), y = lmer.d, colour = factor(sd.random))) +
  geom_quasirandom(dodge.width = 1/2, alpha = 1/5, size = 2) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  theme(legend.position = "none") +
  ylab("lmer.d - true effect")
p_leg <- get_legend(p2 + geom_point(alpha = 4/5) + theme(legend.position = "bottom"))
grid.arrange(p1, p2, p_leg, layout_matrix = rbind(c(1,2), c(3)))
```
...
```{r}
p1 <- plot.error.rates(sims.meta, formula("as.numeric(lm > .05) ~ s(n.pseudo) + fixed.effects")) +
  ylab("Type 2 error rate, lm") +
  ylim(0, 1)
p2 <- plot.error.rates(sims.meta, formula("as.numeric(lmer > .05) ~ s(n.pseudo) + fixed.effects")) +
  ylab("Type 2 error rate, lmer") +
  ylim(0, 1)
p3 <- plot.error.rates(sims.meta, formula("as.numeric(lm > .05) ~ s(n.replicates) + fixed.effects")) +
  ylab("Type 2 error rate, lm") +
  ylim(0, 1)
p4 <- plot.error.rates(sims.meta, formula("as.numeric(lmer > .05) ~ s(n.replicates) + fixed.effects")) +
  ylab("Type 2 error rate, lmer") +
  ylim(0, 1)
...
n.replicates <- floor(runif(6, 6, 12)/2)*2 # even number between 6 and 40
n.pseudo <- sample(c(3:6), 6, replace = TRUE)
sd.error <- 1
random.assignment = FALSE
sd.random <- sample(c(0.5, 1, 2), 6, replace = TRUE)
fixed.effects <- 0  # no difference between groups
...
p2 <- plot.error.rates(sims.meta, formula("as.numeric(lmer < .05) ~ s(n.pseudo) + sd.random")) +
  ylab("Type 1 error rate, lmer") +
  ylim(0, 0.15)
p3 <- plot.error.rates(sims.meta, formula("as.numeric(lmer_slope < .05) ~ s(n.pseudo) + sd.random")) +
  ylab("Type 1 error rate, lmer_slope") +
  ylim(0, 0.15)
p4 <- plot.error.rates(sims.meta, formula("as.numeric(lm < .05) ~ s(n.replicates) + sd.random")) +
  ylab("Type 1 error rate, lm") +
  ylim(0, 0.15)
p5 <- plot.error.rates(sims.meta, formula("as.numeric(lmer < .05) ~ s(n.replicates) + sd.random")) +
  ylab("Type 1 error rate, lmer") +
  ylim(0, 0.15)
p6 <- plot.error.rates(sims.meta, formula("as.numeric(lmer_slope < .05) ~ s(n.replicates) + sd.random")) +
  ylab("Type 1 error rate, lmer_slope") +
  ylim(0, 0.15)
...
sims.meta %>%
  summarise(type2.lm = mean(lm > .05), type2.lmer = mean(lmer > .05), type2.lmer_slope = mean(lmer_slope > .05))
```
...
p2 <- plot.error.rates(sims.meta, formula("as.numeric(lmer > .05) ~ s(n.pseudo) + sd.random")) +
  ...
  ylim(0, 0.5)
p3 <- plot.error.rates(sims.meta, formula("as.numeric(lmer_slope > .05) ~ s(n.pseudo) + sd.random")) +
  ylab("Type 2 error rate, lmer_slope") +
  ylim(0, 0.5)
p4 <- plot.error.rates(sims.meta, formula("as.numeric(lm > .05) ~ s(n.replicates) + sd.random")) +
  ylab("Type 2 error rate, lm") +
  ylim(0, 0.5)
...
p2 <- plot.error.rates(sims.meta, formula("as.numeric(lmer > .05) ~ s(n.pseudo) + fixed.effects")) +
  ylab("Type 2 error rate, lmer") +
  ylim(0, 1)
p3 <- plot.error.rates(sims.meta, formula("as.numeric(lmer_slope > .05) ~ s(n.pseudo) + fixed.effects")) +
  ylab("Type 2 error rate, lmer_slope") +
  ylim(0, 1)
p4 <- plot.error.rates(sims.meta, formula("as.numeric(lm > .05) ~ s(n.replicates) + fixed.effects")) +
  ylab("Type 2 error rate, lm") +
  ylim(0, 1)
p5 <- plot.error.rates(sims.meta, formula("as.numeric(lmer > .05) ~ s(n.replicates) + fixed.effects")) +
  ylab("Type 2 error rate, lmer") +
  ylim(0, 1)
p6 <- plot.error.rates(sims.meta, formula("as.numeric(lmer_slope > .05) ~ s(n.replicates) + fixed.effects")) +
  ylab("Type 2 error rate, lmer_slope") +
  ylim(0, 1)
